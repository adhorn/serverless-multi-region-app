...
USEAST1DOMAIN=$(aws cloudformation describe-stacks --stack-name ${STACKNAME} --region ${REGION2} --output text --query 'Stacks[0].Outputs[?OutputKey==`ServiceEndpoint`].OutputValue')
...
HEALTHCHECK1=$(aws route53 create-health-check  --caller-reference $(date "+%Y%m%d%H%M%S") --health-check-config Type=HTTPS,ResourcePath="/dev/health",FullyQualifiedDomainName="k90kby12nl.execute-api.us-west-2.amazonaws.com",RequestInterval=10,FailureThreshold=1 --profile ${PROFILE} --output text --query 'HealthCheck.Id')
...
HEALTHCHECK2=$(aws route53 create-health-check  --caller-reference $(date "+%Y%m%d%H%M%S") --health-check-config Type=HTTPS,ResourcePath="/dev/health",FullyQualifiedDomainName="k90kby12nl.execute-api.us-west-2.amazonaws.com",RequestInterval=10,FailureThreshold=1 --profile ${PROFILE} --output text --query 'HealthCheck.Id')
